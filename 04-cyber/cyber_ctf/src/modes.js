// The two difficulty modes, shared by the start screen's picker and the
// leaderboard's tabs so the two can't drift apart.
//
// Runs are ranked per mode, never across modes: EASY clicks files and
// HARD types every command, so one mixed board would just sort by mode
