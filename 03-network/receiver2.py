...
Run it:
    ...
    python receiver.py --id PI-02 --led-pin 17     # flash a real LED too
"""
...
def setup_led(led_pin):
    """
    Build the optional breadboard LED. Returns a PacketLED, or None if the
    LED wasn't requested or its helper file is missing. Either way the
    receiver carries on — the light is a bonus, never a requirement.
    """
    if led_pin is None:
        return None  # no --led-pin given, so no hardware at all

    # shared/ lives one folder up from this script. Add it to the search
    # path so "from shared..." works no matter where you run the command.
    sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    try:
        from shared.led_indicator import PacketLED
    except ImportError:
        print("[LED] Couldn't find shared/led_indicator.py — continuing with no light.")
        return None
    return PacketLED(led_pin)
...
def main():
    ...
    parser.add_argument("--led-pin", type=int, default=None,
                        help="GPIO pin for an LED that flashes on each packet "
                             "(e.g. --led-pin 17). Omit it for no LED.")
    ...
    # Optional breadboard LED. None unless you pass --led-pin. Prints its own
    # status line so you can see whether a real light is in play.
    led = setup_led(args.led_pin)
    ...
    try:
        while True:
            ...
            # --- HARDWARE HOOK: flash the breadboard LED -------------------
            # Non-blocking, and a no-op if no LED is wired or enabled, so the
            # network demo is never held up or broken by the hardware.
            if led is not None:
                led.flash()
            # ---------------------------------------------------------------
